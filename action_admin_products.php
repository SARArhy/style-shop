...
if ($uploadOk == 0) {
    ...
    if (move_uploaded_file($_FILES["pro_image"]["tmp_name"], $target_file)) {
        echo "پرونده " . basename($_FILES["pro_image"]["name"]) . " با موفقیت به سرویس دهنده میزبان انتقال یافت  <br  />";
    } else {
...
if ($uploadOk == 1) {
    ...
    if (mysqli_query($link, $query) === true)
        echo ("<p style='color:green;text-align: center;'><b>کالا با موفقیت به انبار اضافه شد</b></p>");
    else
        echo ("<p style='color:red;text-align: center;'><b>خطا در ثبت مشخصات کالا در انبار</b></p>");
} else {
    echo ("<p style='color:red;text-align: center;'><b>خطا در ثبت مشخصات کالا در انبار</b></p>");
}
